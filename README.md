...
| `/api/v1/assistant` | 챗봇용 도구 정의·시스템 프롬프트 중계, 대화 (SSE) |
| `/api/v1/stream` | 화면끼리 공유하는 상태와 확산 프레임 (SSE) |
| `/api/v1/push` | 주민 기기 Web Push 구독 |
...
SSE 는 탭이 살아 있는 동안만이다. 새벽 세 시에 산불이 나면 아무도 앱을 보고 있지 않고,
그때 잠긴 화면을 켜는 것은 브라우저 벤더의 푸시 서비스뿐이다.

```
상황 발령 ──커밋 후──▶ incident.declared ──┬──▶ SSE      (열려 있는 화면)
                                          └──▶ Web Push (닫혀 있는 기기)
```

발표는 **커밋된 뒤에만** 나간다 (`events.publish_after_commit`). 커밋 전에 알리면
롤백된 상황이 주민 화면에 뜨고, 알리지 않고 커밋만 하면 저장은 됐는데 아무도 모른다 —
둘 다 겪었다.

**잠금화면을 켤 만한 것만 보낸다.** 상황 발령과 대피 경로가 막힌 것, 둘뿐이다. 확산
프레임까지 밀면 사람들이 알림을 끄고, 그다음 진짜 경보도 같이 사라진다.

키 한 쌍을 만들어 `.env` 에 넣는다. 없으면 푸시만 꺼지고 나머지는 그대로 동작한다.

```bash
uv run python -m app.cli push-keys
```

비밀키를 가진 사람은 이 도메인 이름으로 주민 잠금화면에 무엇이든 띄울 수 있다. 저장소에
커밋하지 않는다.

죽은 구독(404·410)은 전송할 때마다 지운다. 남겨 두면 명단이 유령으로 불어나 실제
도달률을 가린다.
